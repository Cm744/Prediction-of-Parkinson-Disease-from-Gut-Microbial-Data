...
legend('bottomleft', c('AUPRC', 'AUC'),col=c('red', 'black'),lty=c(4,1),bty = "o",cex=0.8)
##
dev.off()
...
legend('bottomright', c('OTU+age+sex', 'age+sex','baseline'),col  =c('blue', 'darkgreen','red'),cex=0.8,lty=c(1,2,3),lwd=2,bg="white")
##
dev.off()
...
mtext(side=2, line=3, "Absolute Value Coefficients of Genura Features of Elastic LR", cex=1)
abline (v=seq(1.5,25,by = 1), col = "lightgrey", lty=2,lwd=2)
grid()
legend('topright', c('-positive', '-negative'),fill =c('red', 'blue'),bty = "o",cex=0.8,bg="white")
dev.off()
...
labels <- top_genera[1:25]
text(seq(1,25,by=1), par("usr")[3] - 0.012, srt = 25, adj = 1, labels =labels, xpd = TRUE,cex=0.6)
mtext(side=2, line=3, "Coefficients of Genura Features of Elastic LR", cex=1)
abline (v=seq(1.5,25,by = 1), col = "lightgrey", lty=2,lwd=2)
...
mtext(side=2, line=3, "Standardlized Log Relative Abundance of Elastic LR", cex=1)
