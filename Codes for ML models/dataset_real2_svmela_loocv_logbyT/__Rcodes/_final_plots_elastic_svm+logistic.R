...
#########Error raete#######
pdf(file=paste0("./__final_plots/elasticSVM+logistic/elastic_SVM+logistic_Error_Rate.pdf"),width=6,height = 6)
par(mar=c(3,4,1,4))
...
mtext(side=1, line=2, expression(log(lambda)), cex=1)
abline(h=seq(0.2,0.55,by=0.005),v=seq(-12,-4,by=1),col="lightgrey",lty=3)
legend('bottomleft', c('OTU+age+sex', 'age+sex','baseline'),col  =c('blue', 'darkgreen','red'),cex=0.8,lty=c(1,2,3),lwd=2,bg="white")
##
dev.off()

#########amlp#######
pdf(file=paste0("./__final_plots/elasticSVM+logistic/elastic_SVM+logistic_amlp.pdf"),width=6,height = 6)
par(mar=c(3,4,1,4))
...
#########Probability of Parkinson#######
pdf(file=paste0("./__final_plots/elasticSVM+logistic/Probability of Parkinson.pdf"),width=6,height = 6)
par(mar=c(4,4,2,2))
par(mfrow=c(1,1))
PDorder<-order(y_true)
fr<-data.frame(p=pred_matrix[sample_order,,best_lam.n], l=y_true)
#plot(pred_matrix[sample_order,2,best_lam.n],pch=2*y_true[sample_order]-1,col=-2*y_true[PDorder]+6,ylab="Probability of Parkinson",xlab="Subject ID")
...
), paste0('age+sex, AUC=',round(AUC_non_otu,2))),col  =c('blue', 'darkgreen'),bty = "o",cex=0.8,lty=c(1,2),lwd=2)
dev.off()
######### P_R curve #########
...
legend('topright', c(paste0('OTU+age+sex, AUPR=',round(AU_PR[best_lam.n],2)), 
                     paste0('age+sex, AUPR=',round(AU_PR_non_otu,2) ),'baseline'),col  =c('blue', 'darkgreen','red'),bty = "o",cex=0.8,lty=c(1,2,3),lwd=2)
dev.off()
...
pdf(file=paste0("./__final_plots/elasticSVM+logistic/top 25 beta_absolute of elasticSVM+logistic.pdf"),width=6,height = 6)
...
mtext(side=2, line=3, "Absolute Value Coefficients of Genura Features ofelastic_SVM+logistic", cex=0.8)
...
#top 20 beta
pdf(file=paste0("./__final_plots/elasticSVM+logistic/top 20 beta_absolute of elasticSVM+logistic.pdf"),width=12,height = 3.5)
...
mtext(side=2, line=3, "Absolute Values of Coefficients", cex=0.8)
...
dev.off()
#top 10 beta
pdf(file=paste0("./__final_plots/elasticSVM+logistic/top 10 beta_absolute of elastic_SVM+logistic.pdf"),width=6,height = 6)
...
mtext(side=2, line=3, "Absolute Value Coefficients of Genura Features of elastic_SVM+logistic", cex=0.8)
...
pdf(file=paste0("./__final_plots/elasticSVM+logistic/boxplot of top 25 normalized OTU vs parkinson elasticSVM+logistic.pdf"),width=6,height = 6)
...
mtext(side=2, line=3, "Standardlized Log Relative Abundance of elastic_SVM+logistic", cex=1)
abline (v=seq(3,72,by = 3), col = "grey", lty=2,lwd=2)
grid()
legend('topright', c('-parkinson', '-control'),fill =c('red', 'blue'),bty = "o",cex=0.7,bg='white')
dev.off()

pdf(file=paste0("./__final_plots/elasticSVM+logistic/boxplot of top 20 normalized OTU vs parkinson elasticSVM+logistic.pdf"),width=12,height = 4)
...
mtext(side=2, line=3, "Standardlized Log Relative Abundances", cex=1)
...
pdf(file=paste0("./__final_plots/elasticSVM+logistic/boxplot of top 15 normalized OTU vs parkinson elasticSVM+logistic.pdf"),width=6,height = 6)
...
pdf(file=paste0("./__final_plots/elasticSVM+logistic/boxplot of top 10 normalized OTU vs parkinson elasticSVM+logistic.pdf"),width=6,height = 6)
